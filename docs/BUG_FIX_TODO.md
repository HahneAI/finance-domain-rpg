...
DW-1 through DW-8 are all fixed — see the Fixed table below. DW-9 is the one
open finding, still awaiting its fix:

| # | Finding | Where | Severity / blast radius | Warden entry |
|---|---------|-------|------------------------|--------------|
| DW-9 | ✅ **Coach's rate limiter and cost guardrails aren't durable — both quietly assume the current isAdmin/isTester-only population.** `shouldFireForTier()`'s fire-history (`coachTriggers.js`) is keyed to `localStorage` (`coachNetWorthSignal`), not a per-account Supabase column, so it resets on a new device, a cleared cache, or a PWA reinstall — a real user could re-receive a tier message it already "used up." Separately, there's no per-feature token/cost telemetry anywhere in `api/coach.js` (flagged as a day-one §18 item in `docs/TODO.md`, still unbuilt) — nothing would surface a runaway prompt as a graph before it becomes a surprise invoice. Neither is a live defect today only because the gated population is a handful of vetted admins/testers; both become real production risk the moment `canAccessAiFeatures({isAdmin, isTester})` stops being the ceiling on who can open Ask Coach. Fix shape: durable per-account rate-limit state (needs §18.H's `coach_chats` table or an equivalent column) + basic per-call cost logging, both landed *before* the isAdmin/isTester gate is loosened. | `src/lib/coachTriggers.js` (`shouldFireForTier`) · `api/coach.js` (no cost telemetry) · `App.jsx` (Ask Coach gate) | **D4** — gate drift, same class as DW-8: the guardrails were sized to the current gate's population, not to what happens once that gate opens; not wrong today, but drifts into a real gap the instant the gate changes. Pre-launch blocker, not yet a live defect | Flagged 2026-07-22 during Ask Coach general-rollout readiness review; `docs/TODO.md` §18.B/§18.C already note the two gaps individually but hadn't been cross-filed here — not yet covered by a formal warden pass |
...
|---|---------|-------|------------------------|-----|
| DW-1 | **Quick Rate Update didn't eager-save the live rate.** `onActivate` set `config.baseRate` via bare `setConfig` — no `savePersistedStateNow` — so the live rate rode the 800ms debounce. The `account_history` row + optimistic `baseRateHistory` append were already safe, so week math survived a lost write after reload, but live `config.baseRate` (ProfilePanel display, wizard previews) could silently revert. | `App.jsx` — `RateUpdateModal.onActivate` | Soft-D3 · any user running Quick Rate Update on mobile | Mirrors `JobLossEntry.onActivate`'s compute-then-eager-save shape immediately above it: computes `nextConfig` synchronously, calls `setConfig(nextConfig)` then `savePersistedStateNow({ config: nextConfig })` in the same handler. `drift-app-warden.md` §7.4 finding 1 (T1 pass) |
...
| DW-8 | **`CoachNetWorthCard` (Red tier trigger) could never render during Job Loss Mode.** `App.jsx`'s Home-view branch fully swaps `HomePanel` for `JobLossHomePanel` whenever `config.jobLossMode` is true, and `JobLossHomePanel` never mounted `CoachNetWorthCard` — the Red tier's entire premise (Job Loss Mode + runway under 30 days) was structurally unreachable. | `App.jsx` Home-view branch; `HomePanel.jsx` / `JobLossHomePanel.jsx` | **D4** — gate drift, mode-switch silently dropped the surface instead of carrying it forward | Mounted `CoachNetWorthCard` inside `JobLossHomePanel` too, behind the same `canAccessAiFeatures({isAdmin, isTester})` gate as `HomePanel`'s mount (`App.jsx` now threads `currentWeek`/`isAdmin`/`isTester` to `JobLossHomePanel`). Added a real `includeBenefits` prop to `CoachNetWorthCard` (was hardcoded `true` — dead code until this mount existed) so the Job Loss panels' live benefit-scenario toggle reaches the card instead of a stale default. Amber/Green tiers still won't fire from this mount (they need `netWorthHealth`, a normal-mode-only concept not threaded through) — only Red is reachable here, which matches what Red actually means. Regression-tested in `jobLossFlow.test.jsx` ("Coach presence (DW-8 fix)": gated for non-admin/non-tester, renders Red tier for admin when runway < 30 days). |
