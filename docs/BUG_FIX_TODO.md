...
| DW-7 | ✅ **Lifecycle cron never fetches `is_tester` — tester exemption is dead in production.** `decideLifecycleAction`'s bypass gate (`api/_lifecycleEngine.js:44`) checks `row.is_admin \|\| row.is_investor \|\| row.is_tester`, but the cron's SELECT (`api/cron-subscription-lifecycle.js:135–137`) fetches only `is_admin, is_investor` — `row.is_tester` is always `undefined`, so a beta tester whose 6-month window lapses unrenewed will be dunned on the real cadence and, at window+7d, **archived and deleted** by `archiveAndDeleteAccount` — the exact outcome §23/§20 say must never happen. Unit tests pass because they build rows with `is_tester` set; the select list sits outside the pure engine's test seam. Fix: add `is_tester` to the SELECT; add a shell-level test asserting every column the engine reads appears in the query string. | `api/cron-subscription-lifecycle.js:135–137` vs `api/_lifecycleEngine.js:44` | **D4 · HIGH** — silent auto-deletion of beta tester accounts ~6 months after flag flip | `drift-app-warden.md` §16.4 finding 1 (T9 pass) |
