  ...
  const effectiveBottomNav = useMemo(() => {
    ...
    if (canAccessAskCoachGeneral({ isAdmin, isTester, isInvestor: config.isInvestor, entitlement })) {
      items.push({
        key: "__coach__",
        label: "Coach",
        // Same currentColor pattern as the other 4 BOTTOM_NAV icons above —
        // see components/CoachMonocleIcon.jsx (shared with the per-message
        // sender badge in AskCoachPanel.jsx, single source so they can't drift).
        icon: <CoachMonocleIcon size={22} />,
      });
    }
    return items;
