...
export default function App() {
  ...
  // there, since every unconfirmed week IS the notification). Red = that
  // same unchecked count PLUS anything new since the tester last opened the
  // panel — a new changelog entry, a new suggestion prompt, or an updated
  // score — styled identically to the check-in bell's red badge.
  // "New since last opened" needs an actual read marker (unlike the
  // checklist, nothing else here has its own per-tester "done" state), so
  // it's tracked client-side only via localStorage — the same
  // lastSeenChangelogId pattern the sitewide changelog bell already uses
  // below (closeChangelogModal). No DB round trip, no realtime subscription
  ...
  const loadBetaHomebaseBadge = useCallback(async () => {
    ...
    const [items, completions, suggestions, myScore, changelog] = await Promise.all([
    ...
    if (!lastViewedAt) {
      // First time this device has ever computed the badge for this account —
      // stamp "now" as the baseline instead of flagging every pre-existing
      // changelog entry/suggestion/score as "new," which would otherwise dump
      // a false red badge on the whole cohort the moment this feature ships.
    ...
    const scoreUpdated = myScore?.updated_at && myScore.updated_at > lastViewedAt ? 1 : 0;
  ...
  // other's state.
  const [newJobSeasonIncludeBenefits, setNewJobSeasonIncludeBenefits] = useState(true);
  ...
  const [newJobSeasonBannerDismissed, setNewJobSeasonBannerDismissed] = useState(false);
  ...
  const effectiveBottomNav = useMemo(() => {
    ...
    if (canAccessAskCoachGeneral({ isAdmin, isTester, isInvestor: config.isInvestor, entitlement })) {
      items.push({
        key: "__coach__",
        label: "Coach",
        // src/assets/coach-svg-icon.svg — same currentColor pattern as the other
        // 4 BOTTOM_NAV icons above; native viewBox (0 0 2048 2048) kept as-is,
        // width/height scale it down to the same 22px box.
        icon: (
          <svg width="22" height="22" viewBox="0 0 2048 2048" fill="currentColor">
            <path d="M 200.873 178.343 C 223.843 183.501 266.844 198.956 290.269 206.806 L 426.154 251.695 L 861.484 396.096 L 1421.66 583.434 C 1525.96 617.608 1629.73 653.352 1732.95 690.653 L 1806.95 716.894 C 1822.81 722.399 1841.13 728.137 1856.54 734.277 C 1854.86 735.245 1853.41 736.17 1851.78 737.205 C 1820.2 739.185 1775.45 735.446 1743.49 733.41 C 1654.15 727.722 1564.99 713.338 1476.44 700.94 C 1446.82 696.792 1401.37 687.201 1373.08 685.67 C 1378.9 691.99 1397.44 704.068 1405.2 709.643 C 1415.88 717.399 1426.25 725.584 1436.27 734.179 C 1456.29 751.09 1472.64 768.726 1490.52 787.847 C 1634.3 941.688 1689.66 1163.24 1626.65 1365.21 C 1583.35 1501.57 1494.18 1618.74 1374.28 1696.82 C 1218.11 1799.12 1028.76 1825.74 850.616 1769.52 C 690.879 1720.43 557.695 1608.99 481.209 1460.41 C 410.499 1321.9 397.666 1160.99 445.529 1013.02 C 493.771 863.935 596.543 738.57 733.271 662.023 C 763.039 645.225 794.576 632.044 826.082 618.922 C 831.921 616.49 837.758 614.073 843.36 611.125 L 844.508 610.508 C 831.465 607.7 819.12 601.904 806.6 597.465 C 621.063 531.682 464.09 407.783 316.841 280.185 L 246.309 218.875 C 230.843 205.208 216.815 191.835 200.873 178.343 z M 1061.62 1636.54 C 1308.14 1621.62 1495.95 1409.78 1481.24 1163.25 C 1466.53 916.716 1254.85 728.722 1008.31 743.222 C 761.463 757.74 573.19 969.727 587.921 1216.56 C 602.651 1463.39 814.799 1651.48 1061.62 1636.54 z"/>
            <path d="M 1017.71 1035.68 C 1102.95 1026.14 1179.8 1087.45 1189.43 1172.67 C 1199.06 1257.9 1137.83 1334.82 1052.61 1344.54 C 967.268 1354.27 890.213 1292.93 880.568 1207.58 C 870.923 1122.22 932.347 1045.23 1017.71 1035.68 z"/>
            <path d="M 1856.54 734.277 C 1857.56 734.708 1859.28 735.093 1859.49 736.377 C 1858.72 737.441 1853.14 737.152 1851.78 737.205 C 1853.41 736.17 1854.86 735.245 1856.54 734.277 z" fillOpacity="0.94902"/>
          </svg>
        ),
      });
    }
  ...
  // paycheck (TODO §1 New Job Season investigation, 2026-07-19). For a
  ...
  // never wired at all before, so a New Job Season user asking Coach about
  ...
  const coachRunwayDays = useMemo(
    () => resolvePrimaryRunwayDays(newJobSeasonDash, config, newJobSeasonIncludeBenefits),
    [newJobSeasonDash, config, newJobSeasonIncludeBenefits],
  );
  ...
  function handleWizardComplete(mergedConfig) {
    ...
    // §1.H3: a first-run signup that ended in New Job Season skipped the Deductions/
    ...
    <>
      ...
      {currentView === "budget" && (config.newJobSeasonMode ? (
        <NewJobSeasonBudgetPanel
          ...
          effectiveToday={effectiveToday}
          includeBenefits={newJobSeasonIncludeBenefits}
          setIncludeBenefits={setNewJobSeasonIncludeBenefits}
          readOnly={isExpiredReadOnly}
        />
      ) : (
        <BudgetPanel
  ...
  const shellContent = (
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        >
          ...
          {/* ── Beta Tester Homebase — tracked beta testers only, sits directly
              ...
              Badge: green digit = unchecked checklist items only. Red digit =
        ...
        <div ref={mainContentCallbackRef} className="main-content" style={{ padding: "18px 16px", flex: 1, minHeight: 0 }}>
          ...
          {config.newJobSeasonMode && !newJobSeasonBannerDismissed && (() => {
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: "180px" }}>
                ...
                <div style={{ fontSize: "11px", color: "var(--color-text-secondary)", marginTop: "2px" }}>
                  ...
                  {config.startedUnemployed === true ? (
                    "Started your New Job Season — no prior pay history."
                  ) : (
                    <>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                ...
                <Pressable
                  onClick={() => setNewJobSeasonBannerDismissed(true)}
          ...
          {/* TODO §1 mode rebuild (2026-07-18) — the standalone pinned dashboard
              card is gone; its content now lives in NewJobSeasonHomePanel/
              NewJobSeasonBudgetPanel, which render in place of the normal Home/Budget
      ...
      {isAdmin && inspectedWeek && (() => {
        ...
        // §15.I — mirrors buildYear's inNewJobSeason window boundary (finance.js) so the
        ...
        return (
          ...
          >
            ...
            >
              ...
              {inNewJobSeasonWindow && w.unemploymentIncome === 0 && (
                <Row label="Unemployment" val="New Job Season — outside benefit window" mono={false} color="var(--color-text-disabled)" />
              )}
      ...
      */}
      ...
      <LifeEventMenu
        ...
        onSelect={(route) => {
          if (route === "new_job_season") setNewJobSeasonEntryOpen(true);
      ...
      <NewJobSeasonEntry
        ...
        onClose={() => setNewJobSeasonEntryOpen(false)}
        expenses={expenses}
        config={config}
      ...
      <RateUpdateModal
        ...
        onActivate={(patch) => {
          ...
          // NewJobSeasonEntry's onActivate just above).
