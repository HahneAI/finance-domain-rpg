...
const MARKDOWN_COMPONENTS = {
  p:  ({ children }) => <p style={{ fontSize: "13px", color: "var(--color-text-primary)", lineHeight: 1.7, margin: "0 0 12px" }}>{children}</p>,
  ...
  li: ({ children }) => <li style={{ fontSize: "13px", color: "var(--color-text-primary)", lineHeight: 1.6 }}>{children}</li>,
  h1: ({ children }) => <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--color-text-primary)", marginTop: "16px", marginBottom: "8px" }}>{children}</div>,
  h2: ({ children }) => <div style={{ fontSize: "11px", letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--color-teal)", marginTop: "16px", marginBottom: "6px" }}>{children}</div>,
  h3: ({ children }) => <div style={{ fontSize: "11px", letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--color-text-secondary)", marginTop: "14px", marginBottom: "6px" }}>{children}</div>,
  a:  ({ children, href }) => <a href={href} target="_blank" rel="noreferrer" style={{ color: "var(--color-teal)" }}>{children}</a>,
  code: ({ children }) => <code style={{ fontFamily: "var(--font-mono)", background: "var(--color-bg-raised)", padding: "1px 5px", borderRadius: "4px", fontSize: "12px" }}>{children}</code>,
    ...
    <div className="fold-backdrop" data-fold={fold.fold} style={{ position: "fixed", inset: 0, zIndex: 240, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
      <div className="fold-modal" data-fold={fold.fold} style={{ width: "100%", maxWidth: "460px", maxHeight: "80vh", overflowY: "auto", background: "var(--color-bg-surface)", border: "1px solid var(--color-border-accent)", borderRadius: "16px", padding: "22px", display: "flex", flexDirection: "column", gap: "12px" }}>
        <div>
          ...
          {publishedLabel && (
            <div style={{ fontSize: "11px", color: "var(--color-text-secondary)", marginTop: "6px" }}>{publishedLabel}</div>
          )}
        </div>
