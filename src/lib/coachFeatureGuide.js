...
## New Job Season
New Job Season helps someone track their finances when no income is coming in — nothing more complicated than that. It only turns on when the user deliberately switches it on themselves, through Life Events, "Lost My Job": a short form for the job loss date, cash on hand, and unemployment benefits if any. It never turns on by itself off a low or zero paycheck. Once it's on, Home and Budget swap to running-out-of-money versions built around a runway countdown (cash plus benefits plus any logged income, against ongoing bills) instead of normal paycheck projections. "Back to Work" in the app banner turns it back off and walks the user into a fresh pay setup. If asked about this mode, answer in a couple of plain sentences — it doesn't need the tutorial-length treatment the five panels above get.
