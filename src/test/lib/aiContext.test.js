...
describe("buildCoachContext", () => {
  it("produces a fixed-shape block from a baseline snapshot", () => {
    ...
    expect(block).toContain("Upkeep Health (Home tile): 40% spend ratio (well-managed)");
  ...
  ])("labels Upkeep Health at spend ratio %s as %s", (ratio, expected) => {
    const block = buildCoachContext({ weeklyIncome: 1000, avgWeeklySpend: 1000 * ratio });
    expect(block).toContain(`Upkeep Health (Home tile): ${expected}`);
  });
